    ...
    <div className='min-h-screen flex justify-center pt-16 bg-gray-200'>
      <div className={`bg-white p-6 rounded-lg shadow-lg w-full max-w-xl h-full transition-opacity duration-1000 ${fadeIn ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="space-x-4 flex justify-center">
          <Link to="/generator">
            <button className="w-full sm:w-auto bg-blue-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-blue-600 transition primary-button-dark-mode">Generar Contraseña</button>
          </Link>
        
          <Link to="/login">
            <button className="w-full sm:w-auto bg-gray-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-gray-600 transition secondary-button-dark-mode">Iniciar Sesión</button>
          </Link>
        </div>
      </div>
    </div>
